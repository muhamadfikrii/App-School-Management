<div class="overflow-x-hidden" loading="lazy">
    ...
    <section class="relative py-16 lg:py-24 bg-white overflow-hidden">
        ...
        <div class="container mx-auto px-4 sm:px-6 relative z-10">
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 xl:gap-20 items-center">
                <!-- LEFT COLUMN - Enhanced with more visual elements -->
                    ...
                    <div class="ml-4 sm:ml-8 md:ml-12 lg:ml-20 bg-gradient-to-br from-[#161F34] to-[#1E2A4A] text-white p-4 sm:p-6 md:p-8 lg:p-10 relative z-20 rounded-xl shadow-xl overflow-hidden" data-aos="zoom-in" data-aos-delay="200">
                        ...
                        <div class="grid grid-cols-2 gap-4 mb-6 relative z-10">
                            <div class="flex items-center" data-aos="fade-up" data-aos-delay="300">
                                ...
                                <span class="text-xs text-blue-100">Pembelajaran Personal</span>
                            </div>
                            <div class="flex items-center" data-aos="fade-up" data-aos-delay="400">
                                <svg class="w-4 h-4 text-blue-300 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
                                </svg>
                                <span class="text-xs text-blue-100">Kurikulum Modern</span>
                            </div>
                            <div class="flex items-center" data-aos="fade-up" data-aos-delay="500">
                                <svg class="w-4 h-4 text-blue-300 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
                                </svg>
                                <span class="text-xs text-blue-100">Guru Berpengalaman</span>
                            </div>
                            <div class="flex items-center" data-aos="fade-up" data-aos-delay="600">
                                <svg class="w-4 h-4 text-blue-300 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
                                </svg>
                                <span class="text-xs text-blue-100">Fasilitas Lengkap</span>
                            </div>
                        </div>
    ...
    <div class="font-primary bg-white">
        ...
        <script>
            // Enhanced interaction for program items
            document.addEventListener('DOMContentLoaded', function() {
                ...
                // Function to show program content and description
                function showProgramContent(programId) {
                    ...
                    const targetContent = document.getElementById(`${programId}-content`);
                    if (targetContent) {
                        targetContent.classList.remove('hidden');
                        targetContent.classList.add('active');
                    }
                    
                    // Show selected program description
                    const targetDescription = document.getElementById(`${programId}-description`);
                    if (targetDescription) {
                        targetDescription.classList.remove('hidden');
                        targetDescription.classList.add('active');
                    }
                    
                    // Update background gradient based on program
                    const programImage = document.querySelector('.program-image');
                    switch(programId) {
                        case 'tkl':
                            programImage.className = 'program-image rounded-2xl overflow-hidden bg-white/70 aspect-square flex items-center justify-center transition-all duration-500 relative shadow-lg';
                            break;
                        case 'pplg':
                            programImage.className = 'program-image rounded-2xl overflow-hidden bg-white/70 aspect-square flex items-center justify-center transition-all duration-500 relative shadow-lg';
                            break;
                ...
                programItems.forEach(item => {
                    item.addEventListener('click', function() {
                        ...
                        // Show corresponding program content and description
                        const programId = this.getAttribute('data-program');
                        showProgramContent(programId);
                    });
                });
                ...
                document.addEventListener('keydown', function(e) {
                    if (e.key === 'ArrowDown' || e.key === 'ArrowUp') {
                        ...
                        const currentActive = document.querySelector('.program-item[style*="background-color"]');
                        let nextItem;
                        
                        if (e.key === 'ArrowDown') {
                            nextItem = currentActive ? currentActive.nextElementSibling : programItems[0];
                        } else {
                            nextItem = currentActive ? currentActive.previousElementSibling : programItems[programItems.length - 1];
                        }
